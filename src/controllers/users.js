import { generateToken, comparePassword, hashedPassword } from '../helpers/auth';
...
export default class User {
  ...
  static signUp(req, res) {
    try {
      let {
        email, first_name, last_name, address, password, phoneNumber, isAdmin
      } = req.body;
      ...
      db.queryCreate(table, columns, values)
        .then(userRes => {
          ...
          const SignedUp = {
            id, token, first_name, last_name, email, phoneNumber, address, isadmin
          };
          return userResponse(res, 201, ...['status', 201, 'message', 'user Created Successfully', 'data', SignedUp]);
        })
        .catch(err => {
          return serverResponse(res, 403, ...['status', 403, 'error', `This Email Already exists.`]);
        });
    } catch (err) {
  ...
  static UpdateUser(req, res) {
    try {
      ...
      db.userUpdate(table, values, id)
        .then(userRes => {
          return userResponse(res, 200, ...['status', 200, 'message', 'user Updated Successfully', 'data', userRes]);
        })
        .catch(err => {
          return serverError(res);
        });
    ...
    db.querySignin(columns, values)
      .then(response => {
        ...
        if (!decryptedPassword) {
          return serverResponse(res, 422, ...['status', 422, 'error', 'Incorrect Password']);
        }
        const token = generateToken({ id, email, isadmin });

        const loggedIn = {
          id, token, first_name, last_name, email, isadmin
        };

        return userResponse(res, 200, ...['status', 200, 'message', 'Ok', 'data', loggedIn]);
      }).catch(err => {
        return serverError(res);
      })
  }
  ...
  static currentUser(req, res) {
    ...
    db.findCurrentUser(id)
      .then(response => {
        return userResponse(res, 200, ...['status', 200, 'message', 'Ok', 'data', response]);
      })
      .catch(err => {
        return serverError(res);
      });
